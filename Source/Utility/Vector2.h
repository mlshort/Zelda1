...
struct TVector2
{
    ...
    /// Assignment operator
    TVector2<_Ty>& operator= ( const TVector2<_Ty>& rhs ) noexcept
    {
        if (this != &rhs)
        {
            X = rhs.X;
    ...
    TVector2<_Ty>& operator= ( TVector2<_Ty>&& rhs ) noexcept
    {
        if (this != &rhs)
        {
            X = rhs.X;
 ...
 */
    inline TVector2<_Ty>& operator /= (const TVector2<_Ty>& opr ) noexcept
    {
        X /= (opr.X != _Ty() ? opr.X : 1); // don't allow divide by 0
        Y /= (opr.Y != _Ty() ? opr.Y : 1); // don't allow divide by 0
 ...
 */
    inline TVector2<_Ty> Abs( void ) const noexcept
    {
        TVector2<_Ty> vResult(std::abs(X), std::abs(Y));
 ...
 */
    inline TVector2<_Ty> Normal(void) const noexcept
    {
        ...
        {
            _Ty fInverseMagnitude = 1.0 / std::sqrt(fDistanceSquared);
...
typedef math::TVector2<float>       CVector2f;
typedef math::TVector2<double>      CVector2d;
typedef math::TVector2<long double> CVector2ld;
// int based vector2 type definition
typedef math::TVector2<int>         CVector2i;
