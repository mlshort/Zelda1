/**
 *  @file       BlueLeever.cpp
 *  @brief      CBlueLeever class implementation
 *  @author     Mark L. Short
 *  @date       March 12, 2017
 *
...
void CBlueLeever::Render(const CView* pWhere) const
{
    if (m_pTexture && m_pGraphicsDevice)
    {
        bool bResult = m_pTexture->Render(m_pGraphicsDevice->GetRenderer(), 
                                          ...
                                          static_cast<int>(get_PosnY()), get_Direction() );
...
void CBlueLeever::Move(float fDeltaTime) noexcept
{
    ...
    if (nDistance > get_MaxRange())
    {
        ...
        DEGREES degNewDir = std::fmod(degOldDir + 180.0f, 360.0f);
...
void CBlueLeever::Turn(float /*fDeltaTime*/) noexcept
{
}
